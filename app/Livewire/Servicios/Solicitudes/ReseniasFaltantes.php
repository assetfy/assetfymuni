...
class ReseniasFaltantes extends LivewireTable
{
            ...
            ->where(function ($query) use ($serviciosCalificadosIds) {
                $query->whereNotIn('id_solicitud', function ($subQuery) use ($serviciosCalificadosIds) {
                    $subQuery->select('solicitud')
                        ...
                        ->whereIn('id_serviciosActivos', $serviciosCalificadosIds);
                });
            });
        ...
        $columns = [
            Column::make(__('Reseña'), function (Model $model): string {
                ...
                if ($prueba === 'Servicio Realizado, Solicitud Cerrada') {
                    ...
                    if (!$isDisabled) {
                        $reseñaButton = sprintf(
                            '<button 
                                wire:click="$dispatch(\'openDetalleCalificacion\', { data: %d })" 
