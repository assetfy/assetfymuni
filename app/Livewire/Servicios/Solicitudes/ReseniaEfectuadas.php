...
class ReseniaEfectuadas extends LivewireTable
{
    ...
    protected function query(): Builder
    {
        ...
        $serviciosCalificadosIds = CalificacionesModel::pluck('id_serviciosActivos');
        $this->dispatch('openModal', ['servicios.servicios-solicitados-detalle', 'calificaciones.calificacion-servicio', 'calificaciones.vista-calificacion-servicio']);
        $query = $this->model::whereIn('act.solicitudes_servicios.id_activo', $activosIds)
            ->where(function ($query) use ($serviciosCalificadosIds) {
                $query->whereNotIn('id_solicitud', function ($subQuery) use ($serviciosCalificadosIds) {
                    $subQuery->select('solicitud')
                        ->from((new ServiciosActivosModel)->getTable())
                        ->whereNotIn('id_serviciosActivos', $serviciosCalificadosIds);
                });
            });
        ...
        $columns = [
            Column::make(__('Reseña'), function (Model $model): string {
                ...
                if ($prueba === 'Servicio Realizado, Solicitud Cerrada') {
                    ...
                    if ($isDisabled) {
                        $visualizarButton = sprintf(
                            '<button 
                                wire:click="$dispatch(\'openEditarCalificacion\', { data: %d })" 
                                style="background-color: #E5E7EB;"
                                class="text-gray-800 font-bold py-2 px-4 rounded flex items-center justify-center w-12 h-12 transition"
            ...
            Column::make(__('Acciones'), function (Model $model): string {
                return '<button wire:click="$dispatch(\'openDetalleServicio\', { data: ' . $model->getKey() . ' })" 
                    class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-lg flex items-center justify-center gap-2 shadow-md transition h-12"
                            title="Ver Detalle">
