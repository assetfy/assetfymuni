...
class UsuariosActivos extends LivewireTable
{
    ...
    protected $listeners = ['updateEstadoUsuario', 'updateTipoUsuario', 'updateRepresentante', 'updateSupervisor'];
    ...
    protected function query(): Builder
    {
        ...
        return UsuariosEmpresasModel::with('usuarios') // relación con User
            ->where('cuit', $cuit)
            ->where('cargo', '=', 'Empleado')
            ->where('act.usuarios_empresas.estado', '=', 'Aceptado')
    ...
    protected function columns(): array
    {
        return [
            ...
            Column::make(__('Permisos'), function (Model $model): string {
                $currentEstado = $model->estado;
                $isDisabled = ($currentEstado == 'Deshabilitado');
                ...
                if ($model->permisos) {
                    ...
                    $button = '
                            <button 
                                onclick="Livewire.dispatch(\'openModalAsignarRol\', { data: ' . $model->getKey() . ' })"
                                class="' . $buttonClasses . '"
                                ' . ($isDisabled ? 'disabled' : '') . '>
                ...
                ->clickable(false)
                ->asHtml(),
            Column::make(__('Representante tecnico'), function (Model $model): string {
                $currentEstado = $model->estado;
                $isDisabled = ($currentEstado == 'Deshabilitado');
                ...
                if ($model->es_representante_tecnico == 'Si') {
                    $button = '<button 
                        onclick="Livewire.dispatch(\'openModalEditarRepresentante\', { data: ' . $model->getKey() . ' })"
                        class="' . $buttonClasses . '"
                        ' . ($isDisabled ? 'disabled' : '') . '>
                        <i class="fa fa-hard-hat"></i> Editar
                        </button>';
                } else {
                    $button = '<button 
                        onclick="Livewire.dispatch(\'updateRepresentante\', { id: ' . $model->getKey() . ', representante: \'Si\' })"
                        class="' . $buttonClasses . '"
                        ' . ($isDisabled ? 'disabled' : '') . '>
                        <i class="fa fa-user-plus"></i> Activar representante
                        </button>';
                }
