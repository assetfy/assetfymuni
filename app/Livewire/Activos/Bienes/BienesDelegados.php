...
use Carbon\Carbon;
use Illuminate\Support\Collection; // ✅ Importar la clase correcta
use RamonRietdijk\LivewireTables\Columns\ImageColumn;
use App\Helpers\Funciones;

class BienesDelegados extends LivewireTable
{
    protected string $model = ActivosCompartidosModel::class;

    public $title = 'Bienes Delegados'; // Nombre del encabezado
    public $createForm = ''; // Nombre de la función que llama al evento
    public int $tipoId, $userId = 0, $id_activo, $user;
    public $fecha_asignacion, $fecha_fin_asignacion, $fecha_fin;

    protected $listeners = ['removerDelegacion'];
    ...
    protected function query(): Builder
    {
        ...
        $query = $this->model()->query()
            ->where(function ($subquery) {
                $subquery->where('empresa_titular', '=', $this->userId)
        ...
        if (auth()->user()->panel_actual == 'Empresa' || auth()->user()->panel_actual == 'Prestadora') {
            ...
            if ($esApoderado) {
                $query = \App\Models\ActivosCompartidosModel::whereIn('id_activo', $activosIds)
                    ->where('empresa_titular', $this->userId)
                    ->whereNotIn('estado_asignacion', ['Cancelado', 'En Revisión']); // Excluir activos cancelados
            }
        }
    ...
    {
        return [
            ImageColumn::make(__('Imagen'), 'fotos.ruta_imagen')->size(75, 75),
            Column::make(__('Empresa'), 'empresaProveedor.razon_social')
                ->sortable()
                ->searchable(),
            Column::make(__('CUIT'), 'empresaProveedor.cuit')
                ->sortable()
                ->searchable(),
            ...
            Column::make(__('Acciones'), function (Model $model): string {
                ...
                $id_activos_compartidos = $model->getKey();
                $id_activo = $model::where('id_activos_compartidos', $id_activos_compartidos)->pluck('id_activo')->first(); // Usa `first()` para obtener un solo valor
                ...
                ';
                // Construcción de los botones de acción
                return '
    ...
    protected function getTipos()
    {
        $tiposConCategorias = ActivosModel::whereHas('compartidos', function ($query) {
            $query->whereNull('fecha_fin')
                ->where('estado_asignacion', 'Aceptado')
                ->where('empresa_titular', IdHelper::idEmpresa());
        })
