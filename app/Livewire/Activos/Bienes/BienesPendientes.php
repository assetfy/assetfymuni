...
class BienesPendientes extends LivewireTable
{
    protected string $model = ActivosCompartidosModel::class;

    public $title = 'Bienes Pendientes de Aceptación'; // Nombre del encabezado
        ...
        if (auth()->user()->panel_actual == 'Empresa' || auth()->user()->panel_actual == 'Prestadora') {
            ...
            if ($esApoderado) {
                $query = \App\Models\ActivosCompartidosModel::whereIn('id_activo', $activosIds)
                    ->where('empresa_titular', $this->userId)
                    ->whereNotIn('estado_asignacion', ['Aceptado', 'Cancelado']); // Excluir activos cancelados
            }
        }
    ...
    public function removerDelegacion($id)
    {
        $compartidos  = ActivosCompartidosModel::where('id_activos_compartidos', $id)
            ->whereNotIn('estado_asignacion', ['Cancelado'])
            ->whereNull('fecha_fin')
            ->first();
