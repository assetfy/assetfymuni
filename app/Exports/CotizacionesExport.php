...
class CotizacionesExport implements FromCollection, WithHeadings, WithColumnWidths, WithEvents
{
    ...
    public function __construct(Collection $selectedData)
    {
        ...
        $this->data = $selectedData
            ->load(['activos', 'empresasPrestadora']);
    }
    ...
    public function collection()
    {
        return $this->data->map(function ($item) {
            $fechaHora = $item->fecha instanceof Carbon
                ? $item->fechaHora->format('Y-m-d H:i')
                : Carbon::parse($item->fechaHora)->format('Y-m-d H:i');
            return [
                $fechaHora,     //a
                $item->activos->nombre ?? '', // B: 
                $item->Nombre_solicitud  ?? '',                              // C: 
                $item->empresasPrestadora->razon_social  ?? '',                    // D: 
                $item->cotizacion  ?? '',                               // E: 
                $item->garantia_display  ?? '',
                $item->estado_presupuesto                          // F:                     // 
            ];
        });
    }
    /**
