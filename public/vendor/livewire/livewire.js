...
// mode: 'masiva' | 'individual'
window.printMode = function (mode) {
  const html = document.documentElement;
  const source = document.querySelector(
    mode === 'masiva' ? '.print-masiva' : '.print-only-qr'
  );
  if (!source) { console.warn('[print] contenedor no encontrado:', mode); return; }

  // 1) Clonar el contenedor para no mover ni romper bindings
  const clone = source.cloneNode(true);
  clone.id = 'print-root';                // el CSS apunta a este id
  document.body.appendChild(clone);

  // 2) Señal al CSS
  html.setAttribute('data-print', mode);

  // 3) Esperar estilos/recursos y disparar print
