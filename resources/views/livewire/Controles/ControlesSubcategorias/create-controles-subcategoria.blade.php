<div>
    <x-dialog-modal wire:model="open" maxWidth="3xl">
        <x-slot name="title">
            Crear Controles Subcategorias
        </x-slot>
        <form wire:submit.prevent="save">
            <x-slot name="content">
                <!-- Dropdown de Control -->
                <div class="mb-4" x-data="{ openDropdownControl: false }" @click.away="openDropdownControl = false">
                    <x-label value="Control" />
                    <div class="relative">
                        <!-- Botón para abrir/cerrar el dropdown -->
                        <button type="button" @click="openDropdownControl = !openDropdownControl"
                            ...
                            <span>
                                @if ($id_control && $controles->where('id_control', $id_control)->first())
                                    {{ $controles->where('id_control', $id_control)->first()->nombre }}
                                @else
                        ...
                        <div x-show="openDropdownControl" x-transition
                            class="absolute mt-1 w-full rounded-md bg-white shadow-lg z-10">
                            <div class="p-2">
                                <!-- Campo de Búsqueda -->
                                <input type="text" wire:model.live="searchControl"
                                    @keydown.enter="openDropdownControl = false"
                                    @keydown.escape="openDropdownControl = false"
                            ...
                            <ul
                                ...
                                @forelse ($controles as $control)
                                    <li wire:click="setControl({{ $control->id_control }})"
                                        @click="openDropdownControl = false; $wire.set('searchControl', '');"
                                        class="cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-blue-100">
                                        <span class="font-normal block truncate">{{ $control->nombre }}</span>
                                    </li>
                                @empty
                ...
                <div class="mb-4" x-data="{ openDropdownCategoria: false }" @click.away="openDropdownCategoria = false">
                    ...
                    <div class="relative">
                        ...
                        <button type="button"
                            ...
                            {{ !$id_tipo ? 'disabled' : '' }}>
                            <span>
                                @if ($id_categoria && $cat->where('id_categoria', $id_categoria)->first())
                                    {{ $cat->where('id_categoria', $id_categoria)->first()->nombre }}
                                @else
                ...
                <div class="mb-4" x-data="{ openDropdownSubcategoria: false }" @click.away="openDropdownSubcategoria = false">
                    ...
                    <div class="relative">
                        ...
                        <button type="button"
                            ...
                            <span>
                                @if ($id_subcategoria && $subcat->where('id_subcategoria', $id_subcategoria)->first())
                                    {{ $subcat->where('id_subcategoria', $id_subcategoria)->first()->nombre }}
                                @else
                        ...
                        <div x-show="openDropdownSubcategoria" x-transition
                            ...
                            <div class="p-2">
                                <!-- Campo de Búsqueda -->
                                <input type="text" wire:model.debounce.300ms="searchSubcategoria"
                            ...
                            <ul
                                class="max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                                @forelse ($subcat as $subcategoria)
                ...
                <div class="mb-4">
                    ...
                    <select
                        class="form-control w-full border border-gray-300 rounded-md px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500"
                        wire:model.lazy="obligatorio_carga_ini">
                ...
                <div class="mb-4">
                    ...
                    <select
                        class="form-control w-full border border-gray-300 rounded-md px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500"
                        wire:model.lazy="req_foto">
                        <option value="" hidden>Seleccione una Opción</option>
                        <option value="Si">Si</option>
                        <option value="No">No</option>
                    </select>
                    <x-input-error for="req_foto" />
                </div>
                <div class="mb-4">
                    <x-label value="Periodico" />
                    <select
                        class="form-control w-full border border-gray-300 rounded-md px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500"
                        wire:model.lazy="es_periodico">
                        <option value="" hidden>Seleccione una Opción</option>
                        <option value="Si">Si</option>
                        <option value="No">No</option>
                    </select>
                    <x-input-error for="es_periodico" />
                </div>
                <div class="mb-4">
                    <x-label value="Unico" />
                    <x-input type="text"
                        class="w-full border border-gray-300 rounded-md px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500"
                        wire:model.lazy="unico" />
                ...
                <div class="mb-4">
                    ...
                    <x-input type="text"
                        class="w-full border border-gray-300 rounded-md px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500"
                        wire:model.lazy="frecuencia_control"
                        oninput="this.value = this.value.replace(/[^0-9]/g, '')" />
                    <x-input-error for="frecuencia_control" />
                </div>
                <div class="mb-4">
                    <x-label value="Cantidad estandar de control" />
                    <x-input type="text"
                        class="w-full border border-gray-300 rounded-md px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500"
                        wire:model.lazy="cantidad_estandar"
                        oninput="this.value = this.value.replace(/[^0-9]/g, '')" />
                    <x-input-error for="cantidad_estandar" />
                </div>
            </x-slot>
            <x-slot name="footer">
